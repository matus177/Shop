...
class ResetPassword extends CI_Controller
{
            ...
            if ($this->form_validation->run() == false) {
                ...
                if ($userExist) {
                    $this->sendResetPasswordEmail($email) ?
                        $this->session->set_flashdata('category_success', 'Email bol odoslany, prosim skontrolujte aj SPAM zlozku.') :
    ...
    public function resetPasswordForm($email, $emailCode)
    {
        if (isset($email, $emailCode)) {
            ...
            if ($verified) {
                $this->load->view('HeaderView');
                $this->load->view('UpperMenuView');
                $this->load->view('ResetPasswordBodyView', array('emailHash' => $emailHash, 'emailCode' => $emailCode, 'email' => $email));
                $this->load->view('FooterView');
            } else {
    ...
    public function resetPassword()
    {
        ...
        $this->form_validation->set_rules('email', 'Email', 'trim|required|valid_email|xss_clean|max_length[30]');
        $this->form_validation->set_rules('password', 'Heslo', 'trim|required|matches[confpassword]|xss_clean|max_length[30]');
